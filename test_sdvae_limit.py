"""Measure SD-VAE reconstruction ceiling on clean validation images."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test SD-VAE reconstruction limit.")
    parser.add_argument(
        "--data-root",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Hugging Face model id or local path for the SD-VAE weights.",
    )
    return parser.parse_args()


def build_sdvae(model_id: str):
    from ELIR.models.sdvae import SDVAEWrapper

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for this SD-VAE limit test.")

    device = torch.device("cuda")
    model = SDVAEWrapper(model_id=model_id)
...
def main() -> None:
    ...
    vae, device = build_sdvae(args.vae_model_id)
    ...
    print(f"Model: {args.vae_model_id}")
